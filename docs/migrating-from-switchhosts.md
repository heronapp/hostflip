...
1. **Quit SwitchHosts — before hostflip's first launch.** On first launch hostflip takes a snapshot of the current `/etc/hosts` as your read-only *Base Hosts* — the baseline it always applies first and never loses. At that first snapshot, hostflip recognises the block SwitchHosts appends below its `# --- SWITCHHOSTS_CONTENT_START ---` marker and keeps it out of the baseline, so rules that happen to be on do not get welded in — no need to turn them off first. The rules below the marker are not kept either; if you want them, import them from your SwitchHosts data. This recognition applies to the first snapshot only: if SwitchHosts keeps running and writes the file again later, hostflip treats that like any other external edit — a drift review, where accepting the change takes it verbatim. And it relies on the marker, so it does not cover SwitchHosts' non-default overwrite mode. Quitting for good is the reliable path.
